...
// Public self-service password reset was removed for security (an internal
// canteen system should never let an unauthenticated visitor overwrite a
// password from just an email address) — this view only ever renders the
// sign-in form now. The authenticated "Change Password" feature (Profile
// menu, action=change_password) is unrelated and untouched.
...
<body class="login-body" data-view="login">
  ...
  <main class="login-wrap">
    <div class="login-card">
      ...
      <!-- LOGIN PANEL -->
      <section class="login-panel-view" id="panel-login">
        ...
        <?php if ($errorMsg): ?><div class="alert error"><?=View::esc($errorMsg)?></div><?php endif; ?>
        <?php if ($successMsg): ?><div class="alert success"><?=View::esc($successMsg)?></div><?php endif; ?>
  ...
  <script>
    (function () {
      // Only one panel exists now that public password reset is gone, so
      // the view-switching/pushState machinery this used to need for a
      // login<->forgot toggle is gone with it — just the password-visibility
      // toggle and the submit-loading spinner remain.
